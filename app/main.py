    ...
    if uploaded_file is not None:
        ...
        if st.session_state.ingestor is None:
            with st.spinner("Ingesting document ..."):
                ...
                st.session_state.pipeline = RAGPipeline(ingestor.get_vectorstore())
...
user_input = st.chat_input(
    "Ask a question about your document...", disabled=st.session_state.pipeline is None
)
...
if user_input:
    ...
    with st.spinner("Thinking ..."):
        result = st.session_state.pipeline.query(question=user_input, session_id=session_id)
        answer = result["answer"]
        source_docs = result["sources"]

    # Step 4: hallucination check
    status, warning = st.session_state.checker.check(answer, source_docs)
    ...
    st.session_state.messages.append(
        {"role": "assistant", "content": answer, "warning": warning, "sources": formatted_sources}
    )
